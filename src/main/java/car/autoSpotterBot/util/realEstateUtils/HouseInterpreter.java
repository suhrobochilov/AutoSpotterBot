...
public class HouseInterpreter {
    ...
    private final Map<Long, House> currentAd = new ConcurrentHashMap<>();
    private final UserStateRealEstate userStateRealEstate;

    public HouseInterpreter(Button button, BotCallback botCallback, RealEstateService realEstateService, UserStateManager userStateManager, UserStateRealEstate userStateRealEstate) {
        this.button = button;
    ...
    public void interpreter(long chatId, int messageId, String text, String photoUrl, String videoUrl) {
        ...
        House currentHouse = currentAd.getOrDefault(chatId, new House());
        if (text != null) {
            ...
            if (text.equals(ButtonConstant.nextPage)) {
                ...
                realEstateService.displayNextPage(chatId, House.class);
                userStateManager.setUserSubStatus(chatId, PLACE_AD);
            }
            if (text.equals(ButtonConstant.previousPage)) {
                realEstateService.displayPreviousPage(chatId, House.class);
                userStateManager.setUserSubStatus(chatId, PLACE_AD);
            }
            if (text.startsWith(ButtonConstant.nextPhoto) || text.startsWith(ButtonConstant.previousPhoto) ||
                ...
                realEstateService.getNextPhoto(chatId, text, messageId, House.class);
                userStateManager.setUserSubStatus(chatId, PLACE_AD);
            }
            if (text.startsWith(ButtonConstant.favorite)) {
                realEstateService.addToFavorite(chatId, text, House.class);
                userStateManager.setUserSubStatus(chatId, PLACE_AD);
            }
            if (userStateManager.getUserSubStatus(chatId).equals(SEARCH_AD) && !text.equals(ButtonConstant.backInAutoAd)) {
                realEstateService.searchAd(chatId, text, House.class);
            ...
            if (text.equals(ButtonConstant.confirm)) {
                realEstateService.finalizeAndSaveAd(chatId, currentHouse, botCallback);
            ...
            if (!userStateManager.getUserSubStatus(chatId).equals(SEARCH_AD)) {
                switch (text) {
                    case "Toshkent", "Andijon", "Buxoro", "Farg'ona", "Jizzax", "Sirdaryo", "Namangan", "Samarqand",
                        ...
                        realEstateService.setStandort(text, currentHouse);
                        currentAd.put(chatId, currentHouse);
        ...
        if (photoUrl != null) {
            realEstateService.saveUrl(text, photoUrl, null, currentHouse);
            botCallback.deleteMessage(chatId, messageId);

            if (text != null) {
                botCallback.sendPhotoWithInlKeyboard(chatId, currentHouse.getDescription(), photoUrl, button.inlKeyboardConfirmation());
            }
        }

        if (videoUrl != null) {
            realEstateService.saveUrl(text, null, videoUrl, currentHouse);
            if (text != null) {
                botCallback.sendVideoWithInlKeyboard(chatId, currentHouse.getDescription(), videoUrl, button.inlKeyboardConfirmation());
            }
        }
    }
