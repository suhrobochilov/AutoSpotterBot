...
public class HouseInterpreter {
    ...
    public HouseInterpreter(Button button, BotCallback botCallback, UserStateManager userStateManager, UserStateRealEstate userStateRealEstate, GeneralService generalService) {
        this.button = button;
    ...
    public void interpreter(long chatId, int messageId, String text, String photoUrl, String videoUrl) {
        ...
        House currentAd = this.currentAd.getOrDefault(chatId, new House());
        if (text != null) {
            ...
            if (text.startsWith("page")) {
                ...
                generalService.displayNextPage(chatId, House.class, id);
            ...
            if (text.startsWith(ButtonConstant.nextPhoto) || text.startsWith(ButtonConstant.previousPhoto) ||
                ...
                generalService.getNextPhoto(chatId, text, messageId, House.class);
                userStateManager.setUserSubStatus(chatId, PLACE_AD);
            }
            if (text.startsWith(ButtonConstant.favorite)) {
                generalService.addToFavorite(chatId, text, House.class);
                userStateManager.setUserSubStatus(chatId, PLACE_AD);
            }
            if (userStateManager.getUserSubStatus(chatId) != null && userStateManager.getUserSubStatus(chatId).equals(SEARCH_AD) && !text.equals(ButtonConstant.backInAutoAd)) {
                generalService.searchAd(chatId, text, House.class);
            }
    ...
    private void savePhotoUrl(long chatId, int messageId, String text, String photoUrl, House currentAd) {
        if (text != null) {
    ...
    private void saveVideoUrl(long chatId, int messageId, String text, String videoUrl, House currentAd) {
        if (text != null) {
    ...
    private void confirmAd(long chatId, int messageId, House currentAd) {
    ...
    private void saveLocation(long chatId, int messageId, String text, House currentAd) {
        switch (text) {
