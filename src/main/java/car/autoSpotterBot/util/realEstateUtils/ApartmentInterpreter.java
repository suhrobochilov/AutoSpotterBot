...
public class ApartmentInterpreter {
    private static final Logger log = LoggerFactory.getLogger(ApartmentInterpreter.class);
    ...
    public ApartmentInterpreter(Button button, BotCallback botCallback, UserStateManager userStateManager, UserStateRealEstate userStateRealEstate, GeneralService generalService) {
        this.button = button;
    ...
    public void interpreter(long chatId, int messageId, String text, String photoUrl, String videoUrl) {
        ...
        Apartment currentAd = this.currentAd.getOrDefault(chatId, new Apartment());
        if (text != null) {
            ...
            if (text.startsWith("page")) {
                ...
                generalService.displayNextPage(chatId, Apartment.class, id);
            ...
            if (text.startsWith(ButtonConstant.nextPhoto) || text.startsWith(ButtonConstant.previousPhoto) ||
                ...
                generalService.getNextPhoto(chatId, text, messageId, Apartment.class);
                userStateManager.setUserSubStatus(chatId, PLACE_AD);
            }
            if (text.startsWith(ButtonConstant.favorite)) {
                generalService.addToFavorite(chatId, text, Apartment.class);
                userStateManager.setUserSubStatus(chatId, PLACE_AD);
            }
            if (userStateManager.getUserSubStatus(chatId) != null && userStateManager.getUserSubStatus(chatId).equals(SEARCH_AD) && !text.equals(ButtonConstant.backInAutoAd)) {
                generalService.searchAd(chatId, text, Apartment.class);
            }
    ...
    private void savePhotoUrl(long chatId, int messageId, String text, String photoUrl, Apartment currentAd) {
        if (text != null) {
    ...
    private void saveVideoUrl(long chatId, int messageId, String text, String videoUrl, Apartment currentAd) {
        if (text != null) {
    ...
    private void confirmAd(long chatId, int messageId, Apartment currentAd) {
    ...
    private void saveLocation(long chatId, int messageId, String text, Apartment currentAd) {
        switch (text) {
