...
public class BusinessPremiseInterpreter {
    ...
    public BusinessPremiseInterpreter(Button button, BotCallback botCallback, UserStateManager userStateManager, UserStateRealEstate userStateRealEstate, GeneralService generalService) {
        this.button = button;
    ...
    public void interpreter(long chatId, int messageId, String text, String photoUrl, String videoUrl) {
        ...
        BusinessPremise currentAd = this.currentAd.getOrDefault(chatId, new BusinessPremise());
        if (text != null) {
            ...
            if (text.startsWith("page")) {
                ...
                generalService.displayNextPage(chatId, BusinessPremise.class, id);
            ...
            if (text.startsWith(ButtonConstant.nextPhoto) || text.startsWith(ButtonConstant.previousPhoto) ||
                ...
                generalService.getNextPhoto(chatId, text, messageId, BusinessPremise.class);
                userStateManager.setUserSubStatus(chatId, PLACE_AD);
            }
            if (text.startsWith(ButtonConstant.favorite)) {
                generalService.addToFavorite(chatId, text, BusinessPremise.class);
                userStateManager.setUserSubStatus(chatId, PLACE_AD);
            }
            if (userStateManager.getUserSubStatus(chatId) != null && userStateManager.getUserSubStatus(chatId).equals(SEARCH_AD) && !text.equals(ButtonConstant.backInAutoAd)) {
                generalService.searchAd(chatId, text, BusinessPremise.class);
            }
    ...
    private void savePhotoUrl(long chatId, int messageId, String text, String photoUrl, BusinessPremise currentAd) {
        if (text != null) {
    ...
    private void saveVideoUrl(long chatId, int messageId, String text, String videoUrl, BusinessPremise currentAd) {
        if (text != null) {
    ...
    private void confirmAd(long chatId, int messageId, BusinessPremise currentAd) {
    ...
    private void saveLocation(long chatId, int messageId, String text, BusinessPremise currentAd) {
        switch (text) {
