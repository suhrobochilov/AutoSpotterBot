...
@Component
public class BusinessPremiseInterpreter {
    ...
    private final Map<Long, BusinessPremise> currentAd = new ConcurrentHashMap<>();
    private final UserStateRealEstate userStateRealEstate;

    public BusinessPremiseInterpreter(Button button, BotCallback botCallback, RealEstateService realEstateService, UserStateManager userStateManager, UserStateRealEstate userStateRealEstate) {
        this.button = button;
    ...
    public void interpreter(long chatId, int messageId, String text, String photoUrl, String videoUrl) {
        ...
        BusinessPremise currentBusinessPremise = currentAd.getOrDefault(chatId, new BusinessPremise());
        if (text != null) {
            ...
            if (text.equals(ButtonConstant.nextPage)) {
                ...
                realEstateService.displayNextPage(chatId, BusinessPremise.class);
                userStateManager.setUserSubStatus(chatId, PLACE_AD);
            }
            if (text.equals(ButtonConstant.previousPage)) {
                realEstateService.displayPreviousPage(chatId, BusinessPremise.class);
                userStateManager.setUserSubStatus(chatId, PLACE_AD);
            }
            if (text.startsWith(ButtonConstant.nextPhoto) || text.startsWith(ButtonConstant.previousPhoto) ||
                ...
                realEstateService.getNextPhoto(chatId, text, messageId, BusinessPremise.class);
                userStateManager.setUserSubStatus(chatId, PLACE_AD);
            }
            if (text.startsWith(ButtonConstant.favorite)) {
                realEstateService.addToFavorite(chatId, text, BusinessPremise.class);
                userStateManager.setUserSubStatus(chatId, PLACE_AD);
            }
            if (userStateManager.getUserSubStatus(chatId).equals(SEARCH_AD) && !text.equals(ButtonConstant.backInAutoAd)) {
                realEstateService.searchAd(chatId, text, BusinessPremise.class);
            ...
            if (text.equals(ButtonConstant.confirm)) {
                realEstateService.finalizeAndSaveAd(chatId, currentBusinessPremise, botCallback);
            ...
            if (!userStateManager.getUserSubStatus(chatId).equals(SEARCH_AD)) {
                switch (text) {
                    case "Toshkent", "Andijon", "Buxoro", "Farg'ona", "Jizzax", "Sirdaryo", "Namangan", "Samarqand",
                        ...
                        realEstateService.setStandort(text, currentBusinessPremise);
                        currentAd.put(chatId, currentBusinessPremise);
        ...
        if (photoUrl != null) {
            realEstateService.saveUrl(text, photoUrl, null, currentBusinessPremise);
            botCallback.deleteMessage(chatId, messageId);

            if (text != null) {
                botCallback.sendPhotoWithInlKeyboard(chatId, currentBusinessPremise.getDescription(), photoUrl, button.inlKeyboardConfirmation());
            }
        }

        if (videoUrl != null) {
            realEstateService.saveUrl(text, null, videoUrl, currentBusinessPremise);
            if (text != null) {
                botCallback.sendVideoWithInlKeyboard(chatId, currentBusinessPremise.getDescription(), videoUrl, button.inlKeyboardConfirmation());
            }
        }
    }
