...
public class RentalHomeInterpreter {
    private static final Logger log = LoggerFactory.getLogger(RentalHome.class);
    ...
    public RentalHomeInterpreter(Button button, BotCallback botCallback, UserStateManager userStateManager, UserStateRealEstate userStateRealEstate, GeneralService generalService) {
        this.button = button;
    ...
    public void interpreter(long chatId, int messageId, String text, String photoUrl, String videoUrl) {
        ...
        RentalHome currentAd = this.currentAd.getOrDefault(chatId, new RentalHome());
        if (text != null) {
            ...
            if (text.startsWith("page")) {
                ...
                generalService.displayNextPage(chatId, RentalHome.class, id);
            ...
            if (text.startsWith(ButtonConstant.nextPhoto) || text.startsWith(ButtonConstant.previousPhoto) ||
                ...
                generalService.getNextPhoto(chatId, text, messageId, RentalHome.class);
                userStateManager.setUserSubStatus(chatId, PLACE_AD);
            }
            if (text.startsWith(ButtonConstant.favorite)) {
                generalService.addToFavorite(chatId, text, RentalHome.class);
                userStateManager.setUserSubStatus(chatId, PLACE_AD);
            }
            if (userStateManager.getUserSubStatus(chatId) != null && userStateManager.getUserSubStatus(chatId).equals(SEARCH_AD) && !text.equals(ButtonConstant.backInAutoAd)) {
                generalService.searchAd(chatId, text, RentalHome.class);
            }
    ...
    private void savePhotoUrl(long chatId, int messageId, String text, String photoUrl, RentalHome currentAd) {
        if (text != null) {
    ...
    private void saveVideoUrl(long chatId, int messageId, String text, String videoUrl, RentalHome currentAd) {
        if (text != null) {
    ...
    private void confirmAd(long chatId, int messageId, RentalHome currentAd) {
    ...
    private void saveLocation(long chatId, int messageId, String text, RentalHome currentAd) {
        switch (text) {
