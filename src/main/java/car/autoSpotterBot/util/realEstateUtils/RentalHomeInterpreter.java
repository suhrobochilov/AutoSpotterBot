...
@Component
public class RentalHomeInterpreter {
    ...
    private final Map<Long, RentalHome> currentAd = new ConcurrentHashMap<>();
    private final UserStateRealEstate userStateRealEstate;

    public RentalHomeInterpreter(Button button, BotCallback botCallback, RealEstateService realEstateService, UserStateManager userStateManager, UserStateRealEstate userStateRealEstate) {
        this.button = button;
    ...
    public void interpreter(long chatId, int messageId, String text, String photoUrl, String videoUrl) {
        ...
        RentalHome currentRentalHome = currentAd.getOrDefault(chatId, new RentalHome());
        if (text != null) {
            ...
            if (text.equals(ButtonConstant.nextPage)) {
                ...
                realEstateService.displayNextPage(chatId, RentalHome.class);
                userStateManager.setUserSubStatus(chatId, PLACE_AD);
            }
            if (text.equals(ButtonConstant.previousPage)) {
                realEstateService.displayPreviousPage(chatId, RentalHome.class);
                userStateManager.setUserSubStatus(chatId, PLACE_AD);
            }
            if (text.startsWith(ButtonConstant.nextPhoto) || text.startsWith(ButtonConstant.previousPhoto) ||
                ...
                realEstateService.getNextPhoto(chatId, text, messageId, RentalHome.class);
                userStateManager.setUserSubStatus(chatId, PLACE_AD);
            }
            if (text.startsWith(ButtonConstant.favorite)) {
                realEstateService.addToFavorite(chatId, text, RentalHome.class);
                userStateManager.setUserSubStatus(chatId, PLACE_AD);
            }
            if (userStateManager.getUserSubStatus(chatId).equals(SEARCH_AD) && !text.equals(ButtonConstant.backInAutoAd)) {
                realEstateService.searchAd(chatId, text, RentalHome.class);
            ...
            if (text.equals(ButtonConstant.confirm)) {
                realEstateService.finalizeAndSaveAd(chatId, currentRentalHome, botCallback);
            ...
            if (!userStateManager.getUserSubStatus(chatId).equals(SEARCH_AD)) {
                switch (text) {
                    case "Toshkent", "Andijon", "Buxoro", "Farg'ona", "Jizzax", "Sirdaryo", "Namangan", "Samarqand",
                        ...
                        realEstateService.setStandort(text, currentRentalHome);
                        currentAd.put(chatId, currentRentalHome);
        ...
        if (photoUrl != null) {
            realEstateService.saveUrl(text, photoUrl, null, currentRentalHome);
            botCallback.deleteMessage(chatId, messageId);

            if (text != null) {
                botCallback.sendPhotoWithInlKeyboard(chatId, currentRentalHome.getDescription(), photoUrl, button.inlKeyboardConfirmation());
            }
        }

        if (videoUrl != null) {
            realEstateService.saveUrl(text, null, videoUrl, currentRentalHome);
            if (text != null) {
                botCallback.sendVideoWithInlKeyboard(chatId, currentRentalHome.getDescription(), videoUrl, button.inlKeyboardConfirmation());
            }
        }
    }
