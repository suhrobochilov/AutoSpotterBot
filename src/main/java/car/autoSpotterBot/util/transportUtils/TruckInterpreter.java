...
public class TruckInterpreter {
    ...
    private final Map<Long, Truck> currentAd = new ConcurrentHashMap<>();
    private final UserStateTransport userStateTransport;

    public TruckInterpreter(Button button, BotCallback botCallback, GeneralService generalService, UserStateManager userStateManager, UserStateTransport userStateTransport) {
        this.button = button;
    ...
    public void interpreter(long chatId, int messageId, String text, String photoUrl, String videoUrl) {
        ...
        Truck currentAd = this.currentAd.getOrDefault(chatId, new Truck());
        if (text != null) {
            ...
            if (text.startsWith("page")) {
                ...
                generalService.displayNextPage(chatId, Truck.class, id);
            ...
            if (text.startsWith(ButtonConstant.nextPhoto) || text.startsWith(ButtonConstant.previousPhoto) ||
                ...
                generalService.getNextPhoto(chatId, text, messageId, Truck.class);
                userStateManager.setUserSubStatus(chatId, PLACE_AD);
            }
            if (text.startsWith(ButtonConstant.favorite)) {
                generalService.addToFavorite(chatId, text, Truck.class);
                userStateManager.setUserSubStatus(chatId, PLACE_AD);
            }
            if (userStateManager.getUserSubStatus(chatId) != null && userStateManager.getUserSubStatus(chatId).equals(SEARCH_AD) && !text.equals(ButtonConstant.backInAutoAd)) {
                generalService.searchAd(chatId, text, Truck.class);
            }
    ...
    private void savePhotoUrl(long chatId, int messageId, String text, String photoUrl, Truck currentAd) {
        if (text != null) {
    ...
    private void saveVideoUrl(long chatId, int messageId, String text, String videoUrl, Truck currentAd) {
        if (text != null) {
    ...
    private void confirmAd(long chatId, int messageId, Truck currentAd) {
    ...
    private void saveLocation(long chatId, int messageId, String text, Truck currentAd) {
        switch (text) {
