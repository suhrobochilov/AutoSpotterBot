...
public class AgroTechInterpreter {
    ...
    private final Map<Long, AgroTechnology> currentAdMap = new ConcurrentHashMap<>();
    private final UserStateTransport userStateTransport;

    public AgroTechInterpreter(Button button, BotCallback botCallback, TransportService transportService, UserStateManager userStateManager, UserStateTransport userStateTransport) {
        this.button = button;
    ...
    public void interpreter(long chatId, int messageId, String text, String photoUrl, String videoUrl) {
        ...
        if (text != null) {
            ...
            if (text.startsWith(ButtonConstant.favorite)) {
                transportService.addToFavorite(chatId, text, AgroTechnology.class);
                userStateManager.setUserSubStatus(chatId, PLACE_AD);
            }
            if (userStateManager.getUserSubStatus(chatId).equals(SEARCH_AD) && !text.equals(ButtonConstant.backInAutoAd)) {
                transportService.searchAd(chatId, text, AgroTechnology.class);
