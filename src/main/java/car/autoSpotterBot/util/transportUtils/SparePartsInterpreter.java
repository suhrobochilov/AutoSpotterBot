...
public class SparePartsInterpreter {
    ...
    private final Map<Long, SpareParts> currentAd = new ConcurrentHashMap<>();

    public SparePartsInterpreter(Button button, BotCallback botCallback, GeneralService generalService, UserStateManager userStateManager, UserStateTransport userStateTransport) {
        this.button = button;
    ...
    public void interpreter(long chatId, int messageId, String text, String photoUrl, String videoUrl) {
        ...
        SpareParts currentAd = this.currentAd.getOrDefault(chatId, new SpareParts());
        if (text != null) {
            ...
            if (text.startsWith("page")) {
                ...
                generalService.displayNextPage(chatId, SpareParts.class, id);
            ...
            if (text.startsWith(ButtonConstant.nextPhoto) || text.startsWith(ButtonConstant.previousPhoto) ||
                ...
                generalService.getNextPhoto(chatId, text, messageId, SpareParts.class);
                userStateManager.setUserSubStatus(chatId, PLACE_AD);
            }
            if (text.startsWith(ButtonConstant.favorite)) {
                generalService.addToFavorite(chatId, text, SpareParts.class);
                userStateManager.setUserSubStatus(chatId, PLACE_AD);
            }
            if (userStateManager.getUserSubStatus(chatId) != null && userStateManager.getUserSubStatus(chatId).equals(SEARCH_AD) && !text.equals(ButtonConstant.backInAutoAd)) {
                generalService.searchAd(chatId, text, SpareParts.class);
            }
    ...
    private void savePhotoUrl(long chatId, int messageId, String text, String photoUrl, SpareParts currentAd) {
        if (text != null) {
    ...
    private void saveVideoUrl(long chatId, int messageId, String text, String videoUrl, SpareParts currentAd) {
        if (text != null) {
    ...
    private void confirmAd(long chatId, int messageId, SpareParts currentAd) {
    ...
    private void saveLocation(long chatId, int messageId, String text, SpareParts currentAd) {
        switch (text) {
