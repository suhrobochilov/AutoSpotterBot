...
public class OtherTransInterpreter {
    ...
    private final Map<Long, OtherTransport> currentAdMap = new ConcurrentHashMap<>();

    public OtherTransInterpreter(Button button, BotCallback botCallback, TransportService transportService, UserStateManager userStateManager, UserStateTransport userStateTransport) {
        this.button = button;
    ...
    public void interpreter( long chatId, int messageId, String text, String photoUrl, String videoUrl) {
        ...
        OtherTransport currentAd = currentAdMap.getOrDefault(chatId, new OtherTransport());
        if (text != null) {
            ...
            if (text.equals(ButtonConstant.nextPage)) {
                ...
                transportService.displayNextPage(chatId, OtherTransport.class);
            ...
            if (text.startsWith(ButtonConstant.favorite)) {
                transportService.addToFavorite(chatId, text, OtherTransport.class);
                userStateManager.setUserSubStatus(chatId, PLACE_AD);
            }
            if (userStateManager.getUserSubStatus(chatId).equals(SEARCH_AD) && !text.equals(ButtonConstant.backInAutoAd)) {
                transportService.searchAd(chatId, text, OtherTransport.class);
