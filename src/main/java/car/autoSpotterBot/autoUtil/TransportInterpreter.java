...
public class TransportInterpreter {
    ...
    public TransportInterpreter(BotCallback botCallback, BotUserService userService, Button buttonService, StandortService standortService, AutomobileService automobileService, UserStateManager userStateManager, UserStateTrans userStateTrans) {
    ...
    public void autoInterpret(Update update) {
        if (update.hasMessage() && update.getMessage().hasText()) {
            ...
            if (userStateTrans.getUserState(chatId) != null) {
                switch (userStateTrans.getUserState(chatId)) {
                    case AUTO -> autoInterpreter.autointerpreter(update);
            ...
            if (text != null) {
                switch (text) {
                    ...
                    case TransButtonConstant.automobile -> {
                        botCallback.sendMessageWithReplyKeyboard(chatId, "Bo'limni tanlang", button.autoMenu());
                    ...
                    case TransButtonConstant.mainMenu -> {
                        botCallback.sendMessageWithReplyKeyboard(chatId, "Bo'limni tanlang!", button.mainMenu());
            ...
            Automobile currentAutomobile = currentAdAuto.getOrDefault(chatId, new Automobile());
            if (userStateTrans.getUserState(chatId) != null) {
                log.info("CaptionVideo: " + text + " videoUrl: " + videoUrl);
                switch (userStateTrans.getUserState(chatId)) {
                    case AUTO -> autoInterpreter.autointerpreter(update);
                }
            }
            ...
            //validateUserState(chatId, callBackText, currentAutomobile);
            userState.setUserState(chatId, AUTO);
    ...
    private void validateUserState(Long chatId, String text, Automobile currentAutomobile, AgroTechnology currentAgroTech, Truck currentTruck,
        ...
        log.info("Text: " + text + "User State: " + userStateTrans.getUserState(chatId));
        switch (userStateTrans.getUserState(chatId)) {
            case WAITING_FOR_CONFIRMATION -> finalizeAndSaveAd(chatId, currentAutomobile);
    ...
    private void auto(Long chatId) {
        botCallback.sendMessageWithReplyKeyboard(chatId, "Quyidagilardan birini tanlang!", button.transportMenu());
        userState.setUserState(chatId, AUTO);
        photoIndexMap.clear();
    }
